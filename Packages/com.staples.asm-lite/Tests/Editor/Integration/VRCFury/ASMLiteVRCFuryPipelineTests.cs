...
namespace ASMLite.Tests.Editor
{
    ...
    public class ASMLiteVRCFuryPipelineTests
    {
        ...
        [Test, Category("Integration")]
        public void Regression_SurfaceVrcFuryToggle_PreprocessUsesBuildPlannedNameWhenAvatarRootRenamed()
        {
            _ctx.Comp.slotCount = 1;
            ASMLiteTestFixtures.SetExpressionParams(_ctx);
            ...
            int buildResult = ASMLiteBuilder.Build(_ctx.Comp);
            Assert.AreEqual(1, buildResult,
                $"surface VRCFury toggle setup should generate one planned source before preprocess. got {buildResult}.");

            _ctx.AvatarGo.name = _ctx.AvatarGo.name + "(Clone)";

            var callback = new ASMLiteTogglePreprocessAvatarCallback();
            Assert.IsTrue(callback.OnPreprocessAvatar(_ctx.AvatarGo),
                "preprocess enrollment callback should not block VRCFury/SDK preprocessing when Unity appends its prefab clone suffix.");

            Assert.AreEqual(plannedSource, ReadStringField(toggle, "globalParam"),
                "surface toggle preprocess must consume the same planned source name generated by Build; Unity's '(Clone)' suffix must not create a different VRCFury parameter.");

            ASMLiteToggleNameBroker.RestorePendingMutations(warnOnNoData: false);
        }
        ...
        [Test, Category("Integration")]
        public void Regression_FullControllerPrmsParam_IsPlannedAndPreprocessPromotedBeforeVrcFuryBake()
        {
            const string source = "SPS/Lollipop";
            const string backupSlot1 = "ASMLite_Bak_S1_SPS/Lollipop";
            const string backupSlot2 = "ASMLite_Bak_S2_SPS/Lollipop";
            const string defaultKey = "ASMLite_Def_SPS/Lollipop";
            ...
            try
            {
                var fullController = AddVrcFuryFullController(
                    _ctx.AvatarGo,
                    vrcFuryParams,
                ...
                int buildResult = ASMLiteBuilder.Build(_ctx.Comp);
                Assert.AreEqual(1, buildResult,
                    "VRCFury FullController.prms parameters should be planned into generated assets before VRCFury bake, then promoted to globalParams during preprocess.");

                var generatedCtrl = LoadGeneratedController("full controller prms listed param");
                var generatedExpr = LoadGeneratedParams("full controller prms listed param");
                var saveDriver = LoadSlotDriver("full controller prms listed param", generatedCtrl, "ASMLite_Slot1", "SaveSlot1");
                var loadSlot2Driver = LoadSlotDriver("full controller prms listed param", generatedCtrl, "ASMLite_Slot2", "LoadSlot2");

                Assert.IsTrue(generatedCtrl.parameters.Any(p => p.name == source),
                    "regression guard: generated FX controller must declare the FullController.prms source parameter before VRCFury bakes it.");
                Assert.IsTrue(generatedExpr.parameters.Any(p => p != null && p.name == backupSlot1),
                    "regression guard: generated expression params must include a slot 1 backup for the FullController.prms source.");
                Assert.IsTrue(generatedExpr.parameters.Any(p => p != null && p.name == backupSlot2),
                    "repro guard: generated expression params must include a slot 2 backup so loading untouched slot 2 resets the lollipop toggle.");
                Assert.IsTrue(generatedExpr.parameters.Any(p => p != null && p.name == defaultKey),
                    "regression guard: generated expression params must include the Clear Preset default key for the FullController.prms source.");
                Assert.IsTrue(saveDriver.parameters.Any(p => p.type == VRC_AvatarParameterDriver.ChangeType.Copy && p.source == source && p.name == backupSlot1),
                    "save must copy FullController.prms source into the slot 1 backup.");
                Assert.IsTrue(loadSlot2Driver.parameters.Any(p => p.type == VRC_AvatarParameterDriver.ChangeType.Copy && p.source == backupSlot2 && p.name == source),
                    "repro guard: loading untouched slot 2 must copy slot 2 backup/default back into the FullController.prms source.");

                var callback = new ASMLiteTogglePreprocessAvatarCallback();
                Assert.IsTrue(callback.OnPreprocessAvatar(_ctx.AvatarGo),
                    "preprocess enrollment callback should not block VRCFury/SDK preprocessing.");
                Assert.IsTrue(FullControllerGlobalParamsContains(fullController, source),
                    "preprocess enrollment should temporarily add the planned FullController.prms source to globalParams so VRCFury preserves the backed-up name.");

                var restore = ASMLiteToggleNameBroker.RestorePendingFullControllerGlobalMutations(warnOnNoData: false);
                Assert.AreEqual(1, restore.RestoredCount,
                    "preprocess enrollment should leave a restore record for FullController globalParams cleanup.");
                Assert.IsFalse(FullControllerGlobalParamsContains(fullController, source),
                    "restore should return FullController globalParams to the original no-custom-options state.");
            }
            finally
            {
                UnityEngine.Object.DestroyImmediate(vrcFuryParams);
